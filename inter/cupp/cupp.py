...
# We need some informations first!
	...
	name = raw_input("[>] 姓名: 	").lower()
	while len(name) == 0 or name == " " or name == "  " or name == "   ":
		print "\r\n[-] 您必须至少输入一个名称!"
		name = raw_input("[>] Name:    ").lower()
	name = str(name)

	surname = raw_input("[>] 姓:    ").lower()
	nick = raw_input("[>] 小名:    ").lower()
	birthdate = raw_input("[>] 生日(日月年):    ")
	...
	wife = raw_input("[>] 妻子（丈夫）名字:    ").lower()
	wifen = raw_input("[>] 妻子（丈夫）昵称:    ").lower()
	wifeb = raw_input("[>] 妻子（丈夫）生日(日月年):    ")
	...
	kid = raw_input("[>] 小学生的名字：    ").lower()
	kidn = raw_input("[>] 熊孩子的小名：    ").lower()
	kidb = raw_input("[>] 孩子的出生日期(日月年):    ")
	...
	pet = raw_input("[>] 旺财的名字:     ").lower()
	company = raw_input("[>] 公司名称:     ").lower()
	print "\r\n"

	words = ['']
	oth = raw_input("[>] 是否添加一些关键数字思密达？Y/[N]:		 ").lower()
	if oth == "y":
		words = raw_input("[>] 输入请用逗逼号隔开例如[sex. hacker, fuck, 带你装逼.带你飞]: 		").lower().split(", ")

	spechars = ['']
	spechars1 = raw_input("[>] 是否在单词结尾添加特殊字符？ Y/[N]: 		").lower()
	if spechars1 == "y":
		for spec1 in chars:
			spechars.append(spec1)
			for spec2 in chars:
	...
	randnum = raw_input("[>] 是否在单词的结尾加一些随机数吗？ Y/[N]:		").lower()
	leetmode = raw_input("[>] Leet 模式? (i.e. leet = 1337) Y/[N]: 		").lower()
...
# Birthdays first
	...
	print "[+] 已经保存为 \033[1;31m"+name+".txt\033[1;m, 大小为 \033[1;31m"+str(lines)+"\033[1;m words."
	print "[+] 现在你可以尽情玩弄 \033[1;31m"+name+".txt\033[1;m 祝你好运!"
	exit()
